...
hidden1 = Dense(10, activation='gelu',kernel_initializer=tf.keras.initializers.GlorotUniform(seed=17),kernel_regularizer='l2')(visible)
...
predictions_df = pd.DataFrame(predictions, columns = ['i_pred'])
...
sns.set_style('ticks')
plt.figure(dpi=300)
Scatter_S=plt.scatter(y_test_s,y_pred_s, color='black',marker='v',s=12)
...
plt.savefig('ANN3_i_test.pdf', bbox_inches='tight')
plt.show()
...
predictions_train = pd.DataFrame(predictions_train, columns = ['i_pred'])
train = y_train_new_index.join(predictions_train)
#train

y_train_s = train[['i']]
y_train_pred_s = train[['i_pred']]
...
sns.set_style('ticks')
plt.figure(dpi=300)
Scatter_S=plt.scatter(y_train_s,y_train_pred_s, color='black', facecolors='none',marker='v', s=12)
plt.plot( [0,Sv_max],[0,Sv_max],color='firebrick', label='perfect fit' ,lw=1 )
plt.plot( [0,Sv_max],[0,Sv_max*1.2],color='grey', label='+20%', linestyle='--',lw=1 )
plt.plot( [0,Sv_max],[0,Sv_max/1.2],color='grey', label='-20%', linestyle='--',lw=1 )
...
plt.savefig('ANN3_i_train.pdf', bbox_inches='tight')
plt.show()
...
plt.savefig('ANN3_i_SHAP.pdf', bbox_inches='tight')
plt.show()
